/* 
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */

package com.commercesystems.klaviyo_java.models.operations;


import com.commercesystems.klaviyo_java.models.components.CouponCodeCreateQuery;
import com.commercesystems.klaviyo_java.utils.LazySingletonValue;
import com.commercesystems.klaviyo_java.utils.SpeakeasyMetadata;
import com.commercesystems.klaviyo_java.utils.Utils;
import com.fasterxml.jackson.annotation.JsonCreator;
import com.fasterxml.jackson.annotation.JsonIgnore;
import com.fasterxml.jackson.core.type.TypeReference;
import java.lang.Override;
import java.lang.String;
import java.util.Objects;


public class CreateCouponCodeRequest {

    /**
     * API endpoint revision (format: YYYY-MM-DD[.suffix])
     */
    @SpeakeasyMetadata("header:style=simple,explode=false,name=revision")
    private String revision;

    @SpeakeasyMetadata("request:mediaType=application/vnd.api+json")
    private CouponCodeCreateQuery couponCodeCreateQuery;

    @JsonCreator
    public CreateCouponCodeRequest(
            String revision,
            CouponCodeCreateQuery couponCodeCreateQuery) {
        Utils.checkNotNull(revision, "revision");
        Utils.checkNotNull(couponCodeCreateQuery, "couponCodeCreateQuery");
        this.revision = revision;
        this.couponCodeCreateQuery = couponCodeCreateQuery;
    }

    /**
     * API endpoint revision (format: YYYY-MM-DD[.suffix])
     */
    @JsonIgnore
    public String revision() {
        return revision;
    }

    @JsonIgnore
    public CouponCodeCreateQuery couponCodeCreateQuery() {
        return couponCodeCreateQuery;
    }

    public final static Builder builder() {
        return new Builder();
    }

    /**
     * API endpoint revision (format: YYYY-MM-DD[.suffix])
     */
    public CreateCouponCodeRequest withRevision(String revision) {
        Utils.checkNotNull(revision, "revision");
        this.revision = revision;
        return this;
    }

    public CreateCouponCodeRequest withCouponCodeCreateQuery(CouponCodeCreateQuery couponCodeCreateQuery) {
        Utils.checkNotNull(couponCodeCreateQuery, "couponCodeCreateQuery");
        this.couponCodeCreateQuery = couponCodeCreateQuery;
        return this;
    }
    
    @Override
    public boolean equals(java.lang.Object o) {
        if (this == o) {
            return true;
        }
        if (o == null || getClass() != o.getClass()) {
            return false;
        }
        CreateCouponCodeRequest other = (CreateCouponCodeRequest) o;
        return 
            Objects.deepEquals(this.revision, other.revision) &&
            Objects.deepEquals(this.couponCodeCreateQuery, other.couponCodeCreateQuery);
    }
    
    @Override
    public int hashCode() {
        return Objects.hash(
            revision,
            couponCodeCreateQuery);
    }
    
    @Override
    public String toString() {
        return Utils.toString(CreateCouponCodeRequest.class,
                "revision", revision,
                "couponCodeCreateQuery", couponCodeCreateQuery);
    }
    
    public final static class Builder {
 
        private String revision;
 
        private CouponCodeCreateQuery couponCodeCreateQuery;  
        
        private Builder() {
          // force use of static builder() method
        }

        /**
         * API endpoint revision (format: YYYY-MM-DD[.suffix])
         */
        public Builder revision(String revision) {
            Utils.checkNotNull(revision, "revision");
            this.revision = revision;
            return this;
        }

        public Builder couponCodeCreateQuery(CouponCodeCreateQuery couponCodeCreateQuery) {
            Utils.checkNotNull(couponCodeCreateQuery, "couponCodeCreateQuery");
            this.couponCodeCreateQuery = couponCodeCreateQuery;
            return this;
        }
        
        public CreateCouponCodeRequest build() {
            if (revision == null) {
                revision = _SINGLETON_VALUE_Revision.value();
            }            return new CreateCouponCodeRequest(
                revision,
                couponCodeCreateQuery);
        }

        private static final LazySingletonValue<String> _SINGLETON_VALUE_Revision =
                new LazySingletonValue<>(
                        "revision",
                        "\"2024-10-15\"",
                        new TypeReference<String>() {});
    }
}

